...
Для работы с `phi`-узлами был реализован список из 64 регистров типа `i64`. В силу такого подхода никаких оптимизаций использования регистров не было, хотя это и делается достаточно легко с помощью чисел Ершова. 

Также не было задачи написать такой набор инструкций, чтобы он был минимальным, поэтому в `app.asm` вполне может не быть не только инструкций для дебага графического приложения, но и таких как `MOV`, `INC_EQ`... Эти инструкции создавались с точки зрения полноты и симметрии `ISA`.

В проекте реализована кодогенерация инструкций с помщью макроса `ISA`. Поэтому для расширения набора инструкций дописать соответствующий код в файле `./include/ISA.hpp`. 
